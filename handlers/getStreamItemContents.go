...
func GetStreamItemContents(w http.ResponseWriter, r *http.Request) {
	...
	utils.PrintNItems(feedItems, 5)

	// check for form data
	// r.ParseForm()
	// items := r.Form["i"]
	// utils.PrintNItems(items, 5)

	var f fnFeed
	f.ID = "user/-/state/com.google/reading-list"
	f.Updated = time.Now().Unix()

	for _, item := range feedItems {
		f.Items = append(f.Items, fnItem{
			ID:            fmt.Sprintf("tag:google.com,2005:reader/item/%x", item.ID),
			CrawlTimeMsec: fmt.Sprintf("%d", item.LastModified*1000),
			TimestampUsec: fmt.Sprintf("%d000", item.LastModified*1000),
